...
//   const { userData, setChatUser, setMessagesId, getSignedUrl, messagesId, chatVisible, setchatVisible } =
...
const LeftSidebar = () => {
      ...
      const mapped = await Promise.all(
        chatDataArr.map(async (c) => {
          ...
          return {
            ...
            lastMessage: c.lastMessage || "",
            updatedAt: c.updatedAt || null,
            messageSeen: c.messageSeen ?? true,
            username: u.username || u.name || "Unknown",
            avatar: u.avatar || null,
          };
        })
  ...
  const addOrOpenChat = async (otherUser) => {
    ...
    try {
      ...
      const [{ data: meRow }, { data: otherRow }] = await Promise.all([
        supabase.from("chats").select("chatData").eq("id", meId).maybeSingle(),
        supabase.from("chats").select("chatData").eq("id", otherId).maybeSingle(),
      ]);
      ...
      <div className="ls-list">
        {showSearch ? (
          <div className="search-results">
            ...
            {searchUsers.map((u) => (
              <div key={u.id} className="friends" onClick={() => addOrOpenChat(u)}>
                <img src={u.avatar || profile_img} alt="" />
                <p>{u.username}</p>
              </div>
            ))}
            ...
            {localChatList.map((u) => (
              <div key={u.id} className="friends" onClick={() => openChat(u)}>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  <small className={`last-msg ${u.messageSeen ? "" : "unread"}`}>
                    {u.lastMessage}
                  </small>
                </div>
              </div>
